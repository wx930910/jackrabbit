...
public class DiffParserTest extends TestCase {
	...
	public DiffHandler mockDiffHandler14(String value) throws DiffException {
		...
		doAnswer((stubInvo) -> {
			...
			return null;
		}).when(mockInstance).setProperty(any(String.class), any(String.class));
		return mockInstance;
	}
	...
	public DiffHandler mockDiffHandler8() throws DiffException {
		...
		doAnswer((stubInvo) -> {
			...
			return null;
		}).when(mockInstance).addNode(any(String.class), any(String.class));
		doAnswer((stubInvo) -> {
			String targetPath = stubInvo.getArgument(0);
			String diffValue = stubInvo.getArgument(1);
		...
		doAnswer((stubInvo) -> {
			...
			assertEquals("", diffValue);
			return null;
		}).when(mockInstance).remove(any(String.class), any(String.class));
		doAnswer((stubInvo) -> {
			String targetPath = stubInvo.getArgument(0);
			String diffValue = stubInvo.getArgument(1);
			...
			assertEquals("\n", diffValue);
			return null;
		}).when(mockInstance).move(any(String.class), any(String.class));
		return mockInstance;
	}
