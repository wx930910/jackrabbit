...
public class NameParserTest extends TestCase {
	...
	public NamespaceResolver mockNamespaceResolver1() throws NamespaceException {
		...
		when(mockInstance.getPrefix(any(String.class))).thenAnswer((stubInvo) -> {
			...
			if (uri.length() == 0 || uri.indexOf("jackrabbit") > -1 || uri.indexOf("abc") > -1) {
				return uri;
			} else {
		...
		when(mockInstance.getURI(any(String.class))).thenAnswer((stubInvo) -> {
			...
			if (prefix.length() == 0 || prefix.indexOf("jackrabbit") > -1 || prefix.indexOf("abc") > -1) {
				return prefix;
			} else {
	...
	public void testExpandedJcrNames() throws NamespaceException, IllegalNameException, NamespaceException {
		NamespaceResolver resolver = mockNamespaceResolver1();
		...
		valid.add(
				new String[] { "{http://jackrabbit.apache.org/info}abc", "http://jackrabbit.apache.org/info", "abc" });
