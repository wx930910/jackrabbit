...
public class ValueFormatTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		NameResolver nResolver = new ParsingNameResolver(NameFactoryImpl.getInstance(),
	...
	public void testGetPathQValue() throws RepositoryException {
		...
		for (String jcrPath : l) {
			...
			assertEquals("Path values must not be normalized", jcrPath,
					ValueFormat.getJCRValue(qv, resolver, vFactory).getString());
		}
	}
	...
	public void testGetJCRString() throws RepositoryException, URISyntaxException {
		...
		for (QValue qv : qvs) {
			assertEquals(ValueFormat.getJCRValue(qv, resolver, vFactory).getString(),
					ValueFormat.getJCRString(qv, resolver));
		}
	}
}