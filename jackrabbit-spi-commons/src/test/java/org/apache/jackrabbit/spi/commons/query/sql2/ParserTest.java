...
public class ParserTest extends TestCase {
	...
	protected void setUp() throws Exception {
		super.setUp();
		NamePathResolver resolver = new DefaultNamePathResolver(DummyNamespaceResolver.mockNamespaceResolver1());
	...
	private LineNumberReader openScript(String name) {
		try {
			return new LineNumberReader(new InputStreamReader(getClass().getResourceAsStream(name), "UTF-8"));
	...
	public void testFormatLiterals() throws Exception {
		...
		formatLiteral("CAST('2000-01-01T12:00:00.000Z' AS DATE)", "CAST('2000-01-01T12:00:00.000Z' AS DATE)");
		formatLiteral("CAST(0 AS DATE)", "CAST('1970-01-01T00:00:00.000Z' AS DATE)");

		formatLiteral("1", "CAST('1' AS LONG)");
		formatLiteral("-1", "CAST('-1' AS LONG)");
		formatLiteral("CAST(" + Long.MAX_VALUE + " AS LONG)", "CAST('" + Long.MAX_VALUE + "' AS LONG)");
		formatLiteral("CAST(" + Long.MIN_VALUE + " AS LONG)", "CAST('" + Long.MIN_VALUE + "' AS LONG)");

		formatLiteral("1.0", "CAST('1.0' AS DECIMAL)");
		formatLiteral("-1.0", "CAST('-1.0' AS DECIMAL)");
		formatLiteral("100000000000000000000", "CAST('100000000000000000000' AS DECIMAL)");
		formatLiteral("-100000000000000000000", "CAST('-100000000000000000000' AS DECIMAL)");
