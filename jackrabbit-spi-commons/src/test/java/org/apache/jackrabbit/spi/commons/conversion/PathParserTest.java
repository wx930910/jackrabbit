...
public class PathParserTest extends TestCase {
	...
	public NamespaceResolver mockNamespaceResolver1() throws Exception {
		NamespaceResolver mockInstance = Mockito.mock(NamespaceResolver.class);
		Mockito.when(mockInstance.getPrefix(Mockito.any())).thenAnswer((stubInvo) -> {
		...
		Mockito.when(mockInstance.getURI(Mockito.any())).thenAnswer((stubInvo) -> {
