...
public class PathParserTest extends TestCase {
	...
	public NamespaceResolver mockNamespaceResolver1() throws NamespaceException {
		NamespaceResolver mockInstance = mock(NamespaceResolver.class);
		when(mockInstance.getPrefix(any(String.class))).thenAnswer((stubInvo) -> {
		...
		when(mockInstance.getURI(any(String.class))).thenAnswer((stubInvo) -> {
	...
	public void testExpandedPaths() throws Exception, NamespaceException {
		Map<String, Path> paths = new HashMap<String, Path>();

		NameResolver ns = new ParsingNameResolver(NameFactoryImpl.getInstance(), mockNamespaceResolver1());
		...
		paths.put("/a:foo/a:bar",
				PathParser.parse("/{http://jackrabbit.apache.org}foo/{http://jackrabbit.apache.org}bar", ns, factory));
		paths.put("/a:foo/../a:bar", PathParser
				.parse("/{http://jackrabbit.apache.org}foo/../{http://jackrabbit.apache.org}bar", ns, factory));
		paths.put("/foo/..", PathParser.parse("/{}foo/..", ns, factory));
		paths.put("/a:foo/./a:bar", PathParser
				.parse("/{http://jackrabbit.apache.org}foo/./{http://jackrabbit.apache.org}bar", ns, factory));
		paths.put("/a:foo/.././a:bar/./a:foo", PathParser.parse(
				"/{http://jackrabbit.apache.org}foo/.././{http://jackrabbit.apache.org}bar/./{http://jackrabbit.apache.org}foo",
				resolver, factory));
	...
	public void testIdentifierParse() throws RepositoryException {
		...
		for (String id : valid) {
			...
			try {
				...
				// success: invalid argument parent-path if the jcr-path is an identifier-based
				// path.
			}
