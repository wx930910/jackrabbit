...
public class VersionIteratorImplTest extends TestCase {
	...
	public InternalVersion mockInternalVersion1(InternalVersion[] successors, NodeId id) throws Exception {
		NodeId mockFieldVariableId;
		InternalVersion[] mockFieldVariableSuccessors;
		InternalVersion mockInstance = Mockito.mock(InternalVersion.class);
		mockFieldVariableSuccessors = successors;
		mockFieldVariableId = id;
		Mockito.when(mockInstance.getSuccessors()).thenAnswer((stubInvo) -> {
			return Arrays.asList(mockFieldVariableSuccessors);
		});
		Mockito.when(mockInstance.getId()).thenAnswer((stubInvo) -> {
