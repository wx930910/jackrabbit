...
public class UserImporterTest extends AbstractJCRTest {
	...
	public ImportConfig mockImportConfig2(int importBehavior) {
		...
		ImportConfig mockInstance = Mockito.spy(ImportConfig.class);
	...
	public void testActionExecutionForUser() throws Exception {
		AuthorizableAction testAction = Mockito.mock(AuthorizableAction.class);
		...
		Mockito.doAnswer((stubInvo) -> {
			...
			return null;
		}).when(testAction).onCreate(Mockito.any(Group.class), Mockito.any(Session.class));
		Mockito.doAnswer((stubInvo) -> {
			...
			return null;
		}).when(testAction).onCreate(Mockito.any(User.class), Mockito.any(String.class), Mockito.any(Session.class));
		Mockito.doAnswer((stubInvo) -> {
			String newPassword = stubInvo.getArgument(1);
			testActionPw[0] = newPassword;
			return null;
		}).when(testAction).onPasswordChange(Mockito.any(), Mockito.any(), Mockito.any());
