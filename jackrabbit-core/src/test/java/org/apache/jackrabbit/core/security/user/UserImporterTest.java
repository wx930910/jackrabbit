...
public class UserImporterTest extends AbstractJCRTest {
	...
	public ImportConfig mockImportConfig2(int importBehavior) {
		UserImporter[] mockFieldVariableUserImporter = new UserImporter[1];
		ImportConfig mockInstance = Mockito.spy(ImportConfig.class);
		mockFieldVariableUserImporter[0] = createImporter();
		(mockFieldVariableUserImporter[0]).setImportBehavior(ImportBehavior.nameFromValue(importBehavior));
		try {
			Mockito.doAnswer((stubInvo) -> {
				return Collections.singletonList(mockFieldVariableUserImporter[0]);
	...
	public void testActionExecutionForUser() throws Exception {
		AuthorizableAction testAction = Mockito.spy(AuthorizableAction.class);
		...
		try {
			Mockito.doAnswer((stubInvo) -> {
				...
				return null;
			}).when(testAction).onCreate(Mockito.any(User.class), Mockito.any(String.class),
